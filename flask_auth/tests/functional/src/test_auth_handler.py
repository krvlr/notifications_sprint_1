...
async def test_signup(make_post_request, postgre_engine, cases: list[dict]):
    for case in cases:
...
async def test_signin(make_post_request, postgre_engine, cases: list[dict]):
    for case in cases:
        response = await make_post_request(endpoint=case["endpoint"], data=case["request"])
        assert response.status == case["response"]["status"]
        assert response.body.get("success") == case["response"]["success"]
...
async def test_refresh(make_post_request, make_get_request, postgre_engine, cases: list[dict]):
...
async def test_signout(make_post_request, make_get_request, postgre_engine, cases: list[dict]):
    for case in cases["signup"]:
        response = await make_post_request(
            endpoint="signup", data=case["request"], flush_cache=False
        )
...
async def test_signout_all(make_post_request, make_get_request, postgre_engine, cases: list[dict]):
    for case in cases["signup"]:
        response = await make_post_request(
            endpoint="signup", data=case["request"], flush_cache=False
        )
...
async def test_password_change(make_post_request, postgre_engine, cases: list[dict]):
    for case in cases["signup"]:
        response = await make_post_request(
            endpoint="signup", data=case["request"], flush_cache=False
        )
...
async def test_history(make_post_request, make_get_request, postgre_engine, cases: list[dict]):
    for case in cases["signup"]:
        response = await make_post_request(
            endpoint="signup", data=case["request"], flush_cache=False
        )
